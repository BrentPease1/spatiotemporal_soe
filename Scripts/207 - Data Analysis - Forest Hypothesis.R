...
for(fs in 1:nrow(focal_species)){
  ...
  holder <- data.frame(AOU = 0, Year = 0, state_route_grp = 0, forest = 0)
  counter = 0
  ...
  for(yy in min_lc_year:max_lc_year){
    ...
    for(b in 1:nrow(buff)){
      ...
      prop_forest <- sum(values(forest_lc)) / length(values(forest_lc))
      prop_forest <- tibble( AOU = this_buff$AOU,Year = this_buff$Year, state_route_grp = this_buff$state_route_grp, forest = prop_forest)
      holder <- bind_rows(holder, prop_forest)
  ...
  save(holder, file = paste0(here('Results/SoE_BBS/Prop_Forest'), '/SOE_BBS_prop_forest_species_AOU_',focal_species[fs,AOU], '.Rdata'))
